...
class RobomimicAbsoluteActionConverter:
    def __init__(self, dataset_path, algo_name='bc'):
        ...
        self.n_arms = len(env.env.robots)
    ...
    def convert_actions(self, 
        ...
        """
        stacked_actions = actions.reshape(*actions.shape[:-1], self.n_arms, -1)
        ...
        action_goal_ori = np.zeros(
            stacked_actions.shape[:-1]+(3,), 
            dtype=stacked_actions.dtype)
        # might have multiple gripper components (for dexterous hand)
        action_gripper = stacked_actions[..., 6:]
        for i in range(len(states)):
            ...
            for idx, robot in enumerate(env.env.robots):
                ...
                comp_controller = robot.composite_controller

                for controller in comp_controller.part_controllers.values():
                    if isinstance(controller, OperationalSpaceController):
                        # we add origin pos so action is in same frame as obs
                        action_goal_pos[i,idx] = controller.goal_pos + controller.origin_pos
