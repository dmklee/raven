from uferl.model.common.normalizer import SingleFieldLinearNormalizer
from uferl.common.pytorch_util import dict_apply, dict_apply_reduce, dict_apply_split
import numpy as np
...
def robomimic_abs_action_only_dual_arm_normalizer_from_stat2(stat):
    Da = stat["max"].shape[-1]
    Dah = Da // 2
    result = dict_apply_split(
        stat,
    ...
    pos0_param, pos0_info = get_pos_param_info(result["pos0"])
    pos1_param, pos1_info = get_pos_param_info(result["pos1"])
    rot0_param, rot0_info = get_rot_param_info(result["rot0"])
    gripper0_param, gripper0_info = get_gripper_param_info(result["gripper0"])
    rot1_param, rot1_info = get_rot_param_info(result["rot1"])
    gripper1_param, gripper1_info = get_gripper_param_info(result["gripper1"])

    param = dict_apply_reduce(
        [pos0_param, rot0_param, gripper0_param, pos1_param, rot1_param, gripper1_param],
        lambda x: np.concatenate(x, axis=-1),
    )
    info = dict_apply_reduce(
        [pos0_info, rot0_info, gripper0_info, pos1_info, rot1_info, gripper1_info],
        lambda x: np.concatenate(x, axis=-1),
    )
